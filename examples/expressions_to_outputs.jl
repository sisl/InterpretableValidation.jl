...
constrained_series = sample_series(ex1, A1D, 1:N, iid_samples, 1)
a1_series_uniform = sample_series([A1D for i in 1:N],  1:N, iid_samples, 1)
...
constrained_series = sample_series(ex1, A1D, x, gp, 1)
a1_series_uniform = sample_series([A1D for i in 1:N],  x, gp, 1)
p = plot(a1_series_uniform[:x], ylims = (-1,1), label="Unconstrained Series", size = (600,200), legend=:bottomright)
plot!(constrained_series[:x], label = "Constrained Series")
